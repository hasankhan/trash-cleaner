...
class CachePlugin {
    ...
    async beforeCacheAccess(cacheContext) {
        const data = await this._configStore.get(FILE_TOKEN);
        if (data) {
...
class OutlookClient extends EmailClient {
    ...
    async _callApi(op, endpoint, accessToken, data) {
        ...
        if (data && (op === 'post' || op === 'patch')) {
            response = await axios.default[op](endpoint, data, options);
        } else {
            response = await axios.default[op](endpoint, options);
        }
...
class OutlookClientFactory extends EmailClientFactory {
    ...
    _createPublicClientApp(credentials) {
        const cachePlugin = new CachePlugin(this.configStore);

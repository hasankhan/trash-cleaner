...
class OutlookClientFactory extends EmailClientFactory {
    ...
    async getInstance(reconfig) {
        ...
        try {
            ...
            accessToken = await this._authorize(credentials, reconfig);
        } catch (err) {
     ...
     */
    async _authorize(credentials, reconfig) {
        ...
        */
        const msalConfig = {
            ...
            cache: {
                cachePlugin: {
                    beforeCacheAccess: cachePlugin.beforeCacheAccess.bind(cachePlugin),
                    afterCacheAccess: cachePlugin.afterCacheAccess.bind(cachePlugin)
                }
            },
        };
        ...
        let response = reconfig ?
            await this._getToken(pca) :
            await this._getSilentToken(pca);

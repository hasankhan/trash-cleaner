...
const SAMPLE_FILES = {
    ...
    'llm-providers.json': 'llm-providers.json.sample',
...
Commands:
  ...
  undo [configDir]                undo last action using the action log`);
    ...
    async _validate(configDirPath) {
        ...
        // Check llm-providers.json (optional, required if LLM rules exist)
        const llmProvidersPath = path.join(configDirPath, 'llm-providers.json');
        if (fs.existsSync(llmProvidersPath)) {
            try {
                ...
                issues.push({ file: 'llm-providers.json', level: 'ok', message: `${count} provider(s) configured` });
            } catch (err) {
                issues.push({ file: 'llm-providers.json', level: 'error', message: err.message });
                hasErrors = true;
            }
        } else {
            issues.push({ file: 'llm-providers.json', level: 'info', message: 'Not found (needed for LLM rules)' });
        }
    ...
    async _runInteractive(trashCleaner) {
        ...
        const confirmed = [];
        let bulk = null; // 'yes' or 'no' when user picks Yes All / No All
     ...
     * @returns {Promise<string>} One of 'yes', 'no', 'yes-all', 'no-all'.
     */
    _promptAction(question) {

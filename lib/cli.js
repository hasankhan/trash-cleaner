    ...
    async _validate(configDirPath) {
        ...
        if (!fs.existsSync(keywordsYaml) && !fs.existsSync(keywordsJson)) {
            ...
            try {
                ...
                issues.push({ file: keywordsFile, level: 'error', message: err.message });
                hasErrors = true;
            }
        }
        ...
        if (fs.existsSync(allowlistYaml) || fs.existsSync(allowlistJson)) {
            ...
            try {
                ...
                issues.push({ file: fs.existsSync(allowlistYaml) ? 'allowlist.yaml' : 'allowlist.json', level: 'error', message: err.message });
                hasErrors = true;
            }
        } else {
        ...
        if (fs.existsSync(llmYaml) || fs.existsSync(llmJson)) {
            ...
            try {
                ...
                issues.push({ file: fs.existsSync(llmYaml) ? 'llm-providers.yaml' : 'llm-providers.json', level: 'error', message: err.message });
                hasErrors = true;
            }
        } else {

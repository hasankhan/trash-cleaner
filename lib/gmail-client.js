...
class GmailClientFactory extends EmailClientFactory {
     ...
     * 
     * @param {string} configDirPath The path to configuration.
     */
    constructor(configDirPath) {
        super();
        this.credentialsPath = path.join(configDirPath, FILE_CREDENTIALS);
        this.tokenPath = path.join(configDirPath, FILE_TOKEN);
    }
    ...
    async _authorize(credentials) {
        ...
        if (fs.existsSync(this.tokenPath)) {
            let token = fs.readFileSync(this.tokenPath);

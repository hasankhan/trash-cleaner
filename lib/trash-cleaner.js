...
class TrashKeyword {
     ...
     */
    constructor(value, fields, labels, action = EmailAction.DELETE, type = RuleType.KEYWORD) {
...
class LlmTrashRule extends TrashRule {
     ...
     * @returns {Promise<boolean>} True if the classifier scores above threshold.
     */
    async isMatch(email) {
        ...
        const results = await classify(text, [this.label, 'other']);
        const top = results[0];
        return top.label === this.label && top.score >= this.threshold;
    }
}
    ...
    async readKeywords() {
        ...
        return keywords.map((keyword, index) => {
            try {
                ...
                return new TrashKeyword(keyword.value, fields, labels, keyword.action, type);
            } catch (err) {

...
class TrashKeyword {
     ...
     * @param {string} [llm] LLM provider name (required for type 'llm').
     */
    constructor(value, fields, labels, action = EmailAction.DELETE, type = RuleType.KEYWORD, title = undefined, llm = undefined) {
...
class LlmTrashRule extends TrashRule {
     ...
     * @returns {Promise<boolean>} True if the LLM says it matches.
     */
    async isMatch(email) {
...
class TrashCleaner {
     ...
     * @param {SeenEmailCache} seenCache Optional cache for skipping already-evaluated emails.
     * @param {object} llmProviders Map of provider name → provider config for LLM rules.
     */
    constructor(client, keywords, reporter, allowlist = [], actionLog = null, minAgeDays = null, seenCache = null, llmProviders = {}) {
        this._client = client;
    ...
    async _getUnreadEmails() {
        ...
        try {
            ...
            const emails = await this._client.getUnreadEmails(since);
...
class TrashCleanerFactory {
     ...
     *
     * @returns {Promise<object>} Map of provider name → provider config.
     */
    async readLlmProviders() {
        let providers;
        try {
        ...
        this._validateLlmProviders(providers);
        return providers;
    }
     ...
     *
     * @param {object} providers The parsed providers config.
     */
    _validateLlmProviders(providers) {
        for (const [name, config] of Object.entries(providers)) {
            ...
            if (!config.args.some(arg => arg.includes('{{prompt}}'))) {
                throw new Error(
     ...
     * 
     * @returns {Promise<{keywords: TrashKeyword[], rawKeywords: object[]}>}
     *   The parsed keywords and the raw JSON array for hashing.
     */
    async readKeywords() {
        let rawKeywords;
        try {
        ...
        const keywords = rawKeywords.map((keyword, index) => {
            try {
                ...
                return new TrashKeyword(keyword.value, fields, labels, keyword.action, type, keyword.title, keyword.llm);
            } catch (err) {
    ...
    _validateKeywordsConfig(keywords) {
        ...
        for (let i = 0; i < keywords.length; i++) {
            ...
            if (entry.type === RuleType.LLM) {
                if (typeof entry.llm !== 'string' || entry.llm.trim() === '') {
                    throw new Error(

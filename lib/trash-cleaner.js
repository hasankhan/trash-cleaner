...
class TrashKeyword {
    ...
    constructor(value, fields, labels) {
        if (!value || !fields || !fields.length || !labels || !labels.length) {
...
class KeywordTrashRule extends TrashRule {
    ...
    constructor(keyword) {
        super();
        if (!keyword || !keyword.value || !keyword.fields ||
            !keyword.fields.length || !keyword.labels ||
     ...
     */
    isMatch(email) {
        let found = Object.getOwnPropertyNames(email).some(field => {
            if (!this.fields.all && !this.fields[field]) return;

            return this.regex.test(email[field]);
        })
...
class TrashCleanerFactory {
    ...
    splitAndTrim(string, separator, defaultValue) {
        return (string ? string : defaultValue).split(separator)
            .map(t => t.trim())
            .filter(t => t);
    }
}
